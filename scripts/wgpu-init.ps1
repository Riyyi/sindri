...
$assetBase = "https://github.com/gfx-rs/wgpu-native/releases/download/$WGPU_VERSION"

if (-not (Test-Path $target)) {
	$url = "$assetBase/$name.zip"
	Write-Output "wgpu setup: downloading $url"
	$tmp = Join-Path ([System.IO.Path]::GetTempPath()) ([System.Guid]::NewGuid().ToString())
	New-Item -ItemType Directory -Path $tmp | Out-Null
	try {
		Invoke-WebRequest -MaximumRedirection 5 -Uri $url -OutFile (Join-Path $tmp "wgpu.zip")
		Expand-Archive -Force -Path (Join-Path $tmp "wgpu.zip") -DestinationPath $target
	} finally {
